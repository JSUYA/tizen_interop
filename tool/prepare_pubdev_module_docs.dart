...
class _State {
  ...
  static _State fromJson(Map<String, Object?> json) {
    ...
    return _State(
      ...
      footerTextPath: json['footerTextPath'] as String? ??
...
Future<void> _prepare(Directory rootDir, _Config config) async {
  ...
  final docsDirectory =
      Directory('${rootDir.path}/doc/dartdoc/tizen_$versionId')
        ..createSync(recursive: true);
  ...
  final overviewCategory = _CategorySpec(
    name: 'tizen_$versionId',
    displayName: 'Tizen $version',
    markdownPath: 'doc/dartdoc/tizen_$versionId/overview.md',
  );
  final moduleCategories = <_CategorySpec>[];
  var stateWritten = false;

  try {
    final overviewDocFile =
        File('${rootDir.path}/${overviewCategory.markdownPath}')
          ..parent.createSync(recursive: true)
          ..writeAsStringSync(
            _buildOverviewCategoryMarkdown(version: version, modules: modules),
          );
    generatedFiles.add(overviewDocFile.path);

    for (final module in modules) {
      final category = _CategorySpec(
        name: 'tizen_${versionId}_${module.moduleSlug}',
        displayName: 'Tizen $version / ${module.moduleSlug}',
        markdownPath: 'doc/dartdoc/tizen_$versionId/${module.moduleSlug}.md',
      );
      moduleCategories.add(category);

      final categoryDocFile = File('${rootDir.path}/${category.markdownPath}')
        ..parent.createSync(recursive: true)
        ..writeAsStringSync(
          _buildModuleCategoryMarkdown(
            version: version,
            module: module,
            docsTypeName: _moduleDocsTypeName(version, module),
          ),
        );
    ...
    final footerTextFile = File('${docsDirectory.path}/footer.txt');
    footerTextFile.parent.createSync(recursive: true);
    footerTextFile.writeAsStringSync(
      _buildFooterText(version: version, modules: modules),
    );
    generatedFiles.add(footerTextFile.path);

    final categories = <_CategorySpec>[overviewCategory, ...moduleCategories];
    final state = _State(
      version: version,
      generatedFiles: generatedFiles,
      directoriesToPrune: directoriesToPrune,
...
String _buildDocsLibrary({
  ...
  required _CategorySpec overviewCategory,
  ...
  final buffer = StringBuffer()
    ...
    ..writeln(
      ...
      'topic pages without creating one wrapper library per module.',
    )
    ..writeln('///')
    ..writeln('/// {@category ${overviewCategory.name}}')
...
String _buildModuleCategoryMarkdown({
  required String version,
  required ModuleInfo module,
  required String docsTypeName,
}) {
  final sharedObjects =
      module.sharedObjects.map((value) => '`$value`').join(', ');
  final buffer = StringBuffer()
    ..writeln('# Tizen $version / ${module.moduleSlug}')
    ..writeln()
    ..writeln('Generated docs namespace: `$docsTypeName`.')
    ..writeln()
    ..writeln(
      'Actual runtime entrypoint: `${module.getterName}` from '
      '`package:tizen_interop/$version/tizen.dart`.',
    ...
    ..writeln()
    ..writeln('Shared object(s): $sharedObjects.')
    ..writeln()
  ...
  if (module.callbackReferences.isNotEmpty) {
    buffer
      ..writeln()
      ..writeln('Callback typedefs used here:');
    for (final callbackReference in module.callbackReferences) {
      buffer.writeln('- `${callbackReference.group.callbackType}`');
    }
  }
  ...
  if (module.callbackRelatedMissingSymbols.isNotEmpty) {
    buffer
      ..writeln()
      ..writeln(
        'Some callback symbols are documented through '
        '`package:tizen_interop_callbacks` rather than this namespace class.',
      );
  }
...
String _buildOverviewCategoryMarkdown({
  required String version,
  required List<ModuleInfo> modules,
}) {
  final moduleBullets =
      modules.map((module) => '- `${module.moduleSlug}`').join('\n');

  return '''
# Tizen $version

Generated categorized pub.dev docs for Tizen $version.

Actual runtime import: `package:tizen_interop/$version/tizen.dart`

Docs-only import: `package:tizen_interop/$version/tizen_docs.dart`
...
String _buildFooterText({
  ...
  return 'Generated categorized pub.dev docs for Tizen $version. '
      'Docs library: package:tizen_interop/$version/tizen_docs.dart. '
      'Module categories: ${modules.length}.';
}

String _buildDartdocOptions(List<_State> states) {
  final versions = states.map((state) => state.version).join(', ');
  final categories = states
      .expand((state) => state.categories)
      .map(
        (category) => '    ${category.name}:\n'
            '      markdown: ${category.markdownPath}\n'
            '      displayName: ${category.displayName}',
      )
      .join('\n');
  final footerText =
      states.map((state) => '    - ${state.footerTextPath}').join('\n');
  final includeLibraries = LinkedHashSet<String>();
  for (final state in states) {
    includeLibraries.addAll(state.includeLibraries);
  }
  final includes =
      includeLibraries.map((library) => '    - $library').join('\n');
...
# generated docs libraries and this file must exist before publishing.
dartdoc:
  # Group the generated docs namespaces under version/module topic pages.
  categories:
${categories.isEmpty ? '    {}' : categories}
  ...
  # Explain that these are generated categorized docs pages.
  footerText:
${footerText.isEmpty ? '    []' : footerText}

  # Publish only the generated docs libraries.
  include:
$includes
''';
}
