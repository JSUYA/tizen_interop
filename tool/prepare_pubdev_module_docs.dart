...
void _writeMethodWrapper(
  ...
  for (final docLine in _convertMethodDocLines(method.docLines)) {
    buffer.writeln('  $docLine');
  }
...
_StructuredDoc _parseStructuredDoc(List<String> docLines) {
  ...
  for (final rawDocLine in docLines) {
    ...
    for (final expandedLine in expandedLines) {
      ...
      final privlevelMatch =
          RegExp(r'^@privlevel\s*(.*)$').firstMatch(trimmed);
      if (privlevelMatch != null) {
        addParagraph(doc.privilegeLevels, privlevelMatch.group(1)!);
        continue;
      }

      final privilegeMatch =
          RegExp(r'^@privilege\s*(.*)$').firstMatch(trimmed);
      ...
      final remarksMatch =
          RegExp(r'^@remarks?\s*(.*)$').firstMatch(trimmed);
      ...
      final exceptionMatch =
          RegExp(r'^@exception\s*(.*)$').firstMatch(trimmed);
      ...
      final internalMatch =
          RegExp(r'^@internal\s*(.*)$').firstMatch(trimmed);
