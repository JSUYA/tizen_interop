...
## Converting doxygen comments to dartdoc

Use the standalone converter when a Dart file already contains `///` doc
comments in Tizen doxygen style and those comments should be rewritten
in-place into dartdoc-friendly markdown without changing the surrounding code.

```sh
dart run tool/convert_doxygen_to_dartdoc.dart lib/src/bindings/6.0/generated_bindings.dart
```

The script updates only doc comment blocks that look like doxygen-tagged
comments. Files without matching blocks are left unchanged.
